...
#define MAX_COEFFS_PER_THREAD 32
...
int get_threads_per_particle(int nmax, int max_coeffs_per_thread)
{
  return get_next_pow2(1+(nmax-1)/max_coeffs_per_thread);
}

// compute a single legendre coefficient as needed
__host__ __device__
cuFP_t legendre_scalar(int l, int m, cuFP_t x)
{
...
(dArray<cudaParticle> P, dArray<int> I, dArray<cuFP_t> mass,
 dArray<cuFP_t> Afac, dArray<cuFP_t> phi, dArray<cuFP_t> theta, dArray<int> Indx, 
...
__global__ void coefComputeReduceKernel(
 ...
 int l, unsigned int nmax, unsigned int Lmax, dArray<cuFP_t> factorial, PII lohi, int threads_per_particle, int threads_per_m
)
{
  ...
  const int coeffs_per_thread = 1 + (nmax-1) / threads_per_m;
  const int thread_particle_offset = tid % threads_per_particle;
  const int coeff_offset = coeffs_per_thread * (thread_particle_offset%threads_per_m);
  const int m = thread_particle_offset / threads_per_m;
  ...
  for (int i = tid/threads_per_particle; i < N; i += stride) {
    ...
    cosp *= legendre_coefficient * norm;
    sinp *= legendre_coefficient * norm;
    
    for (int n = coeff_offset; n < coeff_offset+coeffs_per_thread; n++) {
      cuFP_t v_partial = (n < nmax) ? coef._v[i*nmax + n] : 0.0;
      local_sums[2*(n-coeff_offset)] += cosp * v_partial;
      local_sums[2*(n-coeff_offset)+1] += sinp * v_partial;
    }
  }
  ...
  // to m=0. We could have each thread write the accumulated coefficients from its portion of
  // shared memory - however this would result in 2*coeffs_per_thread writes for each thread
  // The below approach recruits as many threads as possible to do the writing.
  // This should slightly help performance, but probably not significantly
  /*for (int i = offset_in_block; i += blockDim.x; i < threads_per_particle*coeffs_per_thread) {
    // determine the thread that created this coefficient
    int local_tid = i / coeffs_per_thread;
    int global_tid = local_tid + blockIdx.x * blockDim.x;
    int particle_offset = global_tid % threads_per_particle;
    int m = particle_offset / threads_per_m;
    int coeff_offset = coeffs_per_thread * (particle_offset % threads_per_m);
    int n = coeff_offset + i % coeffs_per_thread;
    if (n < nmax) {
      out._v[blockIdx.x + 2*(m*nmax+n) * gridDim.x] = shared[2*i];
      out._v[blockIdx.x + (2*(m*nmax+n)+1) * gridDim.x] = shared[2*i+1];
    }
  }*/
}
 ...
 int sampT, bool pcavar, bool pcaeof, bool subsamp)
{
  ...
  if (theta_d.capacity() < N) theta_d.reserve(N);
...
void SphericalBasis::determine_coefficients_cuda(bool compute)
{
  ...
  if (initialize_cuda_sph) {
    initialize_cuda();
    thrust::host_vector<cuFP_t> local_fact((Lmax+1)*(Lmax+1));
    for (int l = 0; l <= Lmax; l++) {
      for (int m = 0; m <= l; m++) local_fact[(Lmax+1)*l + m] = factorial(l,m);
    }
    cuS.factorial_d = local_fact;
  ...
  for (int n=0; n<Npacks; n++) {
    ...
    coordKernel<<<gridSize, BLOCK_SIZE, 0, cr->stream>>>
      (toKernel(cr->cuda_particles), toKernel(cr->indx1),
       ...
       toKernel(cuS.theta_d), toKernel(cuS.i_d),
	...
	thrust::counting_iterator<int> index_end(gridSize1*osize*(l+1));
...
void SphericalBasis::multistep_update_cuda()
{
  ...
  for (int olev=mfirst[mdrft]; olev<=multistep; olev++) {
	...
	int threads_per_particle = (l+1) * threads_per_m;
	  ...
	  coefComputeReduceKernel
            <<<gridSize1, coefBlockSize, redSMemSize, cs->stream>>>
            (toKernel(cuS.dc_coef), toKernel(cuS.dN_coef), toKernel(cuS.p_d),
             toKernel(cuS.theta_d), l, nmax, Lmax, toKernel(cuS.factorial_d), cur,
	     threads_per_particle, threads_per_m);
